...
def patch_gen(vol, patch_size, stride=1, nargout=1, rand=False, rand_seed=None):
    ...
    for idx in rng:
        ...
        if nargout == 1:
            yield vol[patch_sub]
            #return vol[patch_sub]
        else:
           yield (vol[patch_sub], patch_sub)
           # return (vol[patch_sub],patch_sub)

def my_patch_gen(vol, patch_size, stride=1, nargout=1, rand=False, rand_seed=None):
    """
    NOT VERY WELL TESTED
    generator of patches from volume
    ...
    for idx in rng:
        ...
        # print(patch_sub)
        if nargout == 1:
            vollist.append( vol[patch_sub])
            return vollist
        else:
            vollist.append(vol[patch_sub])
            loclist.append(patch_sub)
            
            return (vollist,loclist)
