...
def build_loss(similarity_type, similarity_scales, regulariser_type, regulariser_weight,
               ...
               bidir=None, ddf_inv=None,label_moving_inv=None,label_fixed_inv=None ):
    if bidir is None:
        label_similarity = multi_scale_loss(label_fixed, label_moving, similarity_type.lower(), similarity_scales)
    else:
        label_similarity = multi_scale_loss(label_fixed, label_moving, similarity_type.lower(), similarity_scales) + \
                           multi_scale_loss(label_fixed_inv, label_moving_inv, similarity_type.lower(), similarity_scales)
    ...
    if network_type.lower() == 'global':
        ...
        if bidir is None:
            ...
            ddf_regularisation = tf.reduce_mean(local_displacement_energy(ddf, regulariser_type, regulariser_weight)) + \
                                 tf.reduce_mean(local_displacement_energy(ddf_inv, regulariser_type, regulariser_weight))
...
def build_loss_t1t2(similarity_type, similarity_scales, regulariser_type, regulariser_weight,
    ...
    if network_type.lower() == 'global':
        ...
        ddf_regularisation = tf.reduce_mean(local_displacement_energy(ddf, regulariser_type, regulariser_weight))+ \
                             ...
                             tf.reduce_mean(tf.square(ddf-ddf_t1) +tf.square(ddf-ddf_t2) +tf.square(ddf_t1-ddf_t2))
...
def double_dice_simple(ts, ps, eps_vol=1e-6):
    numerator = tf.reduce_sum(ts*ps, axis=[1, 2, 3, 4]) * 2- tf.reduce_sum((1-ts)*ps, axis=[1, 2, 3, 4])-tf.reduce_sum(ts*(1-ps), axis=[1, 2, 3, 4])
    denominator = tf.reduce_sum(ts, axis=[1, 2, 3, 4]) + tf.reduce_sum(ps, axis=[1, 2, 3, 4])+eps_vol
