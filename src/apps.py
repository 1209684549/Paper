...
import huutils as util
import sys
sys.path.append('../ext/neuron/neuron')
from utils import  interpn
def warp_volumes_by_ddf_AD(input_, ddf):
    grid_warped = util.get_reference_grid(ddf.shape[1:4]) + ddf
    warped = interpn(tf.convert_to_tensor(input_, dtype=tf.float32), grid_warped,interp_method='nearest')
    with tf.Session() as sess:
        return sess.run(warped)
def warp_volumes_by_ddf_(input_, ddf):
    grid_warped = util.get_reference_grid(ddf.shape[1:4]) + ddf
    warped = interpn(tf.convert_to_tensor(input_, dtype=tf.float32), grid_warped, interp_method='linear')
    with tf.Session() as sess:
        return sess.run(warped)
